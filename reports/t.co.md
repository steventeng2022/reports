...
| 2 | medium | X2 | CORS origin reflection (with credentials on /1) | CWE-942 |
...
- **CWE:** CWE-319
- **Detail:** http://t.co/ returns 520 (Cloudflare Pingora-Origin error page) without redirecting to HTTPS and without an HSTS header; plain-HTTP clients get a Cloudflare error page.
- **Recommendation:** Add an HTTP->HTTPS redirect (currently returns an error code on port 80).

### 2. [MEDIUM] CORS origin reflection (with credentials on /1) (`X2`)

- **CWE:** CWE-942
- **Detail:** t.co reflects an arbitrary Origin into Access-Control-Allow-Origin: on https://t.co/ (200) without credentials, and on short-link requests such as https://t.co/1 (200) WITH Access-Control-Allow-Credentials: true. A hostile origin can read short-link resolution responses cross-origin with credentials; strongest CORS finding of the hunt so far.
...
- **CWE:** CWE-200
- **Detail:** Server header reveals: cloudflare Pingora-Origin
- **Context:** http response
