...
| 9 | low | R2 | No HTTP->HTTPS redirect (legacy domain, 404 on plain HTTP) | CWE-319 |
...
### 9. [LOW] No HTTP->HTTPS redirect (legacy domain, 404 on plain HTTP) (`R2`)

- **CWE:** CWE-319
- **Detail:** Verified: http://support.office.com/ returns 404 (text/html) with no Location header and no HSTS; https://support.office.com/ returns 307 (redirect to the current support portal). The legacy domain is wound down (plain-HTTP root 404s), so the missing HTTP->HTTPS redirect has limited exposure.
